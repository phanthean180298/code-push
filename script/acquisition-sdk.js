...
var AcquisitionManager = /** @class */ (function () {
  ...
  AcquisitionManager.prototype.queryUpdateWithCurrentPackage = function (
    ...
    if (!currentPackage || !currentPackage.appVersion) {
      throw new code_push_error_1.CodePushPackageError(
    ...
    this._httpRequester.request(
      ...
      function (error, response) {
        ...
        if (response.statusCode !== 200) {
          ...
          callback(
            new code_push_error_1.CodePushHttpError(errorMessage),
            /*remotePackage=*/ null
          );
          return;
        }
      ...
      switch (status) {
        ...
        default:
          if (callback) {
            if (!status) {
              callback(
                new code_push_error_1.CodePushDeployStatusError(
    ...
    this._httpRequester.request(
      ...
      function (error, response) {
        if (callback) {
          ...
          if (response.statusCode !== 200) {
            callback(
              new code_push_error_1.CodePushHttpError(
